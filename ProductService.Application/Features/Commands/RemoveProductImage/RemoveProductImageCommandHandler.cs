...
public sealed class RemoveProductImageCommandHandler(
    ...
    TimeProvider timeProvider) : IRequestHandler<RemoveProductImageCommand, IResult>
{
    public async Task<IResult> Handle(RemoveProductImageCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = product.RemoveProductImage(imageUrlResult.Value, timeProvider.GetUtcNow());
        if (result.IsFailure)
            return result;
